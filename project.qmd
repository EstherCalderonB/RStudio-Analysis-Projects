...
muscle_summary <- tidy_muscles %>%
  count(Muscle, sort = TRUE) %>%
  mutate(training_category = case_when(
    n >= 100 ~ "Overtrained",
    n <= 15 ~ "Neglected",
    TRUE ~ "Moderate"
  ))
muscle_summary %>%
  filter(training_category != "Moderate") %>%
  ggplot(aes(x = fct_reorder(Muscle, n), y = n, fill = training_category)) +
  geom_col(show.legend = FALSE) +
  coord_flip() +
  facet_wrap(~training_category, scales = "free_y") +
  labs(
    title = "Overtrained vs. Neglected Muscles",
    x = "Muscle",
    y = "Exercise Count"
  ) +
  theme_minimal()
